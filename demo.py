...
def get_todayevent(filename):
    ...
    for i in range(len(date_event)):
        date_new = xlrd.xldate_as_datetime(date_event[i][2], 0)
        date_new = str(date_new)
        date_new = list(date_new)
        date_new = ''.join(date_new[0:10])#取日期，去掉小时分秒，并拼接成一个字符串
        
        local_time = datetime.datetime.now()
        remind_time_0 = (local_time).strftime("%Y-%m-%d")
        remind_time_0 = str(remind_time_0)
        
        remind_time_1 = (local_time+datetime.timedelta(days=1)).strftime("%Y-%m-%d")
        remind_time_1 = str(remind_time_1)
        
        remind_time_2 = (local_time+datetime.timedelta(days=2)).strftime("%Y-%m-%d")
        remind_time_2 = str(remind_time_2)
        
        remind_time_3 = (local_time+datetime.timedelta(days=3)).strftime("%Y-%m-%d")
        remind_time_3 = str(remind_time_3)
        
        state = date_event[i][3]
        if (remind_time_0 == date_new and state=='未完成') or (remind_time_1 == date_new and state=='未完成') or (remind_time_2 == date_new and state=='未完成') or (remind_time_3 == date_new and state=='未完成') :
